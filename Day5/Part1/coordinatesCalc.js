const calculate = lines => {
    let diagramLength = Math.max(...[...lines.map(line => line.x1), ...lines.map(line => line.x2)]);
    let diagramHeight = Math.max(...[...lines.map(line => line.y1), ...lines.map(line => line.y2)]);

    let diagram = [...Array(diagramHeight)].map(x => Array(diagramLength).fill(0))

    let consideredLines = lines.filter(line => isHorizontal(line));
